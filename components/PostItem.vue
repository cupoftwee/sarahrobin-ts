      ...
      <div class="py-8 sm:py-20 border-b border-gray-300">
        <header class="text-center mb-8">          
          <h2 class="text-3xl sm:text-4xl leading-tight font-sans mb-1 sm:mb-2">
            <g-link :to="`${post.path}/`" class="nav-link text-black font-bold">{{ post.title }}</g-link>
          </h2>
          
          <p class="text-gray-700 leading-normal text-sm sm:text-base">
            <span v-if="post.author">
              by <g-link v-if="post.author" :to="`${post.author.path}/`" class="nav-link text-gray-700 capitalize border-b border-transparent hover:border-gray-400 transition-border-color">
              {{ titleCase(post.author.title) }}</g-link>
            </span>
            
            <span v-if="post.tags && post.tags.length > 0">
              in <g-link :to="`${post.tags[0].path}/`" class="nav-link text-gray-700 capitalize border-b border-transparent hover:border-gray-400 transition-border-color">
              {{ titleCase(post.tags[0].title) }}</g-link>
            </span>
          </p>
          
          <time :datetime="post.datetime" class="text-gray-700 text-xs mb-2 uppercase">
